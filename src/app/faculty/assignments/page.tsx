...
import { collection, onSnapshot, doc, deleteDoc, addDoc, updateDoc, serverTimestamp, getDoc, query, where, setDoc } from 'firebase/firestore';
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-8">
             <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Create, edit, and manage assignments for your classes.</p>
                </div>
             ...
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>A list of all assignments you have posted.</CardDescription>
                </CardHeader>
                <CardContent>
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {assignments.map(a => (
                                <Card key={a.id} className={cn("flex flex-col", isPast(a.dueDate) && !isToday(a.dueDate) ? "opacity-60" : "")}>
                                    ...
                                    <CardFooter className="flex justify-end items-center text-xs pt-4">
                                        <div className="flex items-center gap-1">
                                            <CalendarIcon className="h-3 w-3" /> Due: {format(a.dueDate, 'PPP')}
                                        </div>
                                    </CardFooter>
                                </Card>
                            ))}
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             <div className="space-y-2">
                                ...
                                <Select value={currentAssignment.branch} onValueChange={(v) => setCurrentAssignment({...currentAssignment, branch: v})}>
                                    ...
                                    <SelectContent>{facultyData?.branch?.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                            <div className="space-y-2">
                                <Label>Semester</Label>
                                 <Select value={currentAssignment.semester} onValueChange={(v) => setCurrentAssignment({...currentAssignment, semester: v})}>
                                    ...
                                    <SelectContent>{facultyData?.semesters?.map(s => <SelectItem key={s} value={s}>Semester {s}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                        </div>
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
